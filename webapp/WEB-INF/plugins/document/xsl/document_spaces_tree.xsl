...
<xsl:variable name="current-space"><xsl:value-of select="current-space-id" /></xsl:variable>
<xsl:template match="spaces">
<div id="lutece-tree" class="lutece-tree">
    <ul>
...
<xsl:variable name="index"><xsl:value-of select="space-id" /></xsl:variable>
    <li class="lutece-tree-item" id="node-{$index}" data-tree-icon="{space-icon-url}">
